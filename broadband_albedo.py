...
data  = xlrd.open_workbook('C:/Users/sdh/Desktop/WRF_verify/WW_ALBEDO.xlsx')
# read seperate data
sheet = data.sheets()[0] #Select table (which tables are shown in brackets)
# Generate fake data
ALBEDO_50    = sheet.col_values(1) [1:]#表示取第0-第365行
ALBEDO_200   = sheet.col_values(3) [1:]
ALBEDO_500   = sheet.col_values(5) [1:]
ALBEDO_1000  = sheet.col_values(7) [1:]
#########################################
fig         = plt.figure(figsize=(7,3))#设置图形界面的大小
ax          = fig.add_subplot(111)
####################################
ax.plot(ALBEDO_50,   label='ALBEDO_50',     color='b', linewidth=1.5)
ax.plot(ALBEDO_200,  label='ALBEDO_200',    color='r', linewidth=1.5)
# ax.plot(ALBEDO_500,  label='ALBEDO_500',    color='y', linewidth=1.5)
# ax.plot(ALBEDO_1000, label='ALBEDO_1000',   color='g', linewidth=1.5)
######################################
